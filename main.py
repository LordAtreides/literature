...
def archive_to_sheet(entry, sheet_type="archive"):
    """sheet_type: 'archive' (8+ kayitlar), 'web' (4-7 arasi kayitlar)"""
    if not GOOGLE_SHEET_ID: return
    client = get_gsheets_client()
    if not client: return
    try:
        sh = client.open_by_key(GOOGLE_SHEET_ID)
        # Eger 'web' tablosu yoksa hata verebilir, varsayilan olarak sheet1
        worksheet = sh.sheet1
...
def main():
    ...
    for item in scored_items:
        ...
        if sc >= 8: 
            ...
            web_items.append(item) # 8+ olanlar web'e de kaydedilecek
        elif sc >= 4: 
            web_items.append(item)
        else: 
            trash_items.append(item)
        
    logger.info("ROUTING: %d Telegram (8+), %d Web (4-10), %d Cöp (1-3)", len(telegram_items), len(web_items), len(trash_items))

    # WEB (4-10 Puanlari Google Sheets'e kaydet)
    for item in web_items:
        # Telegram'a gidenleri belli etmek icin tipini degistir
        sheet_type = "telegram" if item.get("score", 0) >= 8 else "web"
        archive_to_sheet(item, sheet_type=sheet_type)
